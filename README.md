...
为了看看后端的情况（我们简单地将含有JavaScript的记录滤掉），单独来观察后端程序员的情况。但输出关联规则为空，说明后端各语言技术栈之间无强关联。这和我们先前作文本分析得到的后端壁垒更为严重的猜想相符。

**结论3**（验证猜想2） 前端技术栈一致性更强，而后端技术栈壁垒更加严重。

接下来我们通过网络图或者桑基图来观察各类语言尤其是后端语言之间的流向。
...
上面流向的结果，还可以通过桑基图来进行反映，利用基于pyecharts的动态桑基图，我们能够较为清晰地说明流量关系。
...
在上个部分中已经提到，前端界技术认知具有一致性，所以TypeScript没有产生较多的讨论。与之鲜明对比的是后端，后端技术的更迭往往伴随着大量的讨论。这或许在一定程度上说明了我们所设想的“后端技术壁垒”，这是各个语言社区之间莫衷一是的结果，这个层面的技术茧房或者说语言墙，来自于迁移本身的难度。

Kotlin是Android端平台上替代Java的下一代开发语言，严格来讲不能归于前后端中的一方。但Kotlin的例子和可以用猜想1解释：

关于技术的认知是否一致，与社区中讨论的热度有关联。所以当社区形成稳定的最优实践的时候要注意，社区的活跃度可能也就不存在了。于是我们不妨将猜想1称为编程语言社区的“热度原理”，大家对一项技术的认知越一致，产生的讨论越少。
...
Python、TypeScript、Go、Rust、Kotlin使用人数确实有着显著的增长，Java也确实下降了，说明大家的技术认识总能。

令人感到疑惑的的是，C语言预期使用的人不多，但使用人群并没有大量的下降。这应当和我们的研究的问题没有严格的逻辑关系，C语言这样的底层语言有其不可替代性，所以有一个稳定的使用人群。但C语言在一致性和热度上面的表现，和我们猜想的“热度原理”是相符的，因为集体或者个人来说都很难形成最优实践，所以始终有着讨论的热度。
...
**结论4** 技术栈一致性、最优实践和准入门槛之间有着关联，但在我们的项目中，无法说明形成这三者之间的因果关系。
...
**结论3**（验证猜想2） 前端技术栈一致性更强，而后端技术栈壁垒更加严重。也即我们称为“语言墙”的现象。前者表现为全体技术栈的统一化，称为狭限；后者表现为迁移困难、无相对最优实践带来的讨论和消耗。这比我们预期的现象更加复杂，内涵更为丰富，因而也可以为越多的人提供建议。

**结论4** 前端开发者中入门者普遍更多，这或许和技术栈的一致性有关系。但其间的因果性我们无法说明。
...
2. “语言墙”存在于两个主要的开发者门类当中
   1. 那些形成了最优实践的语言，要意识到虽然一致性较强，但其门槛较低，自己的技术壁垒很难形成，作为学习者要利用好现有的成果，并居安思危。
   2. 后端技术栈不容易掌握是普遍现象，且各个子技术栈之间的经验流通更加困难。要坚定信心，多参与活跃的开发社区的讨论，构建关于自己的最优实践并不断进步。

## 不足和展望

主要的不足在于，项目中的开发者标签和各种技术标签都是多选，另外还有很多字段为描述性的，所以要构建多对多的相关关系非常困难的，所以在项目中我们没有找到一些关于技术集聚性较强的人群更一般的特征。

另外，由于前、后端开发工程师占开发者占大多数，所以一些人数较少的群体的结论可能本应更显著但被隐藏了。比如我们可以对数据科学工作者的常用语言和技术栈进行案例研究，给数据科学学习者提供一些更加具体的建议，但基于上述4个结论以及我们得到的经验规律，完成这些工作应该不太困难。
